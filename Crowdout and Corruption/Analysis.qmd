...
ggplot(aes(x = as.numeric(year), y = kalahi_spending / pop07nso, color = as.factor(treat))) +
  geom_smooth() +
  geom_point(alpha = .15) +
  guides(color = guide_legend(label = FALSE)) +
  labs(x = "", y = "Public service spending", title = "Figure 1: Balance on pretreatment spending") +
  theme_minimal() +
...
historic_dat |> 
  filter(as.numeric(year) < 2012) |> 
  mutate(treat = ifelse(treat == 1, "treat", "control")) |> 
ggplot(aes(x = as.numeric(year), y = total_spending / pop07nso, color = as.factor(treat))) +
  geom_smooth() +
  geom_point(alpha = .15) +
  labs(x = "Year", y = "Total spending",
       color = "Received Kalahi",
       caption = "Note: Spending measured yearly per capita in Philippine pesos") +
  theme_minimal()
```
